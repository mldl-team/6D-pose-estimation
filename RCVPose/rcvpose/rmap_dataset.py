...
class RMapDataset(Dataset):
    ...
    def __init__(self, root, dname, set, obj_name, kpt_num, transform):
        ...
        if self.dname == 'lm':
            self._imgpath = os.path.join(self.root,'data', self.obj_name, 'rgb', '%06d.png')
            #self._radialpath = os.path.join(self.root, 'LINEMOD_ORIG', self.obj_name, 'Out_pt'+kpt_num+'_dm', '%s.npy')
            self._depthpath = os.path.join(self.root,'data', self.obj_name, 'depth', '%06d.dpt')
            self._maskpath = os.path.join(self.root,'data', self.obj_name, 'mask', '%06d.png')
            self._gtposepath = os.path.join(self.root,'data', self.obj_name, 'pose', 'pose%06d.npy')
            self._imgsetpath = os.path.join(self.root,'data', self.obj_name, 'Split', '%s.txt')

            # load ply 
            #print(self.kpt)        
            cad_model_mm = o3d.io.read_point_cloud(os.path.join(self.root, 'data', self.obj_name, 'mesh.ply'))
            cad_model_points_m = np.asarray(cad_model_mm.points)/1000
            kpt_file = os.path.join(self.root, 'data', self.obj_name, 'Outside9.npy')
            if os.path.isfile(os.path.join(self.root, self.obj_name,'Outside9.npy')):
                ...
                np.save(os.path.join(self.root,"data", self.obj_name,'Outside9.npy'), self.kpt)
    ...
    def __getitem__(self, item):
        ...
        if self.dname == 'lm':
            ...
            depth = read_depth(self._depthpath % int(img_id))
            mask = np.asarray(Image.open(self._maskpath % int(img_id)), dtype=int)[:, :, 0]
            gtpose = np.load(self._gtposepath % int(img_id))
            img = Image.open(self._imgpath % int(img_id)).convert('RGB')
        else:
            #print(self._h5path)
