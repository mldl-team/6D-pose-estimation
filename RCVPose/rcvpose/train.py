...
class Trainer():
    def __init__(self, data_loader, opts, vis = None):
        ...
        if opts.resume_train:
            self.model, self.epoch, self.optim, self.loss_func = utils.load_checkpoint(
                self.model,
                self.optim,
                opts.out + "/model_best.pth.tar"
            )
    ...
    def compute_r_loss(self, pred, gt):
        # ignore all background zeros (gt == 0)
        mask = gt > 0
        num = mask.sum().item()
        if num == 0:
            # no foreground pixels, skip
            return torch.tensor(0.0, device=gt.device)
        # sum L1 over only valid pixels, then average
        loss = self.loss_radial(pred[mask], gt[mask]) / num
        return loss
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, (data, target, sem_target) in tqdm.tqdm(
                ...
                data, target, sem_target = (
                    data.to(self.device),
                    target.to(self.device),
                    sem_target.to(self.device)
                )
                ...
                if self.vis is not None:
                    self.vis.add_scalar('Val_r+s', float(loss.item()), self.iter_val)
                    self.vis.add_scalar('Val_MAE', 
                                        float(torch.sum(torch.abs(score_rad - target)) 
                                              / float(len(torch.nonzero(target)))), 
                                        self.iter_val)
                    self.vis.add_scalar('Val_r', float(loss_r.item()), self.iter_val)
                    self.vis.add_scalar('Val_ACC', 
                                        float(torch.sum(torch.where(
                                            torch.abs(score_rad - target)[target != 0] <= 0.05, 1, 0
                                        )) / float(len(torch.nonzero(target)))), 
                                        self.iter_val)
                    if self.iter_val % 50 == 0:
                        self.vis.add_image('Val_sem',
                                           torch.where(score[0].cpu() >= 0.5, 1, 0),
                                           self.iter_val)
                self.iter_val += 1
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data, target, sem_target) in tqdm.tqdm(
            ...
            if self.vis is not None:
                self.vis.add_scalar('Train_sum', np_loss, self.iteration)
                self.vis.add_scalar('Train_r', loss_r.item(), self.iteration)
                self.vis.add_scalar('Train_s', loss_s.item(), self.iteration)
